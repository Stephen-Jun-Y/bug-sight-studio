...
const buildFallbackDetail = (species: NonNullable<RecognitionResult["species"]>, imageUrl: string): InsectInfo => ({
  id: species.id,
  speciesNameCn: species.name,
  speciesNameEn: species.latinName,
...
const ResultPage = () => {
  ...
  const isUnknown = Boolean(recognition?.isUnknown || !recognition?.species);
  const [expanded, setExpanded] = useState(false);
  const [liked, setLiked] = useState(false);
  const [favoritePending, setFavoritePending] = useState(false);
  const [detail, setDetail] = useState<InsectInfo | null>(() => (
    recognition?.species ? buildFallbackDetail(recognition.species, recognition.imageUrl) : null
  ));
  ...
  useEffect(() => {
    setDetail(recognition?.species ? buildFallbackDetail(recognition.species, recognition.imageUrl) : null);
  }, [recognition]);

  useEffect(() => {
    if (!recognition?.species || speciesId === undefined) return;
    ...
    getSpeciesDetail(speciesId)
      ...
      .catch(() => {
        if (!cancelled) {
          setDetail(buildFallbackDetail(recognition.species, recognition.imageUrl));
        }
      });
  ...
  const speciesName = isUnknown
    ...
    : language === "en-US"
      ? detail?.speciesNameEn || recognition.species?.latinName || t("待补充", "To be added")
      : detail?.speciesNameCn || recognition.species?.name || t("待补充", "To be added");
  const descriptionText = isUnknown
    ? t("未识别到昆虫，请重新拍摄或更换角度", "No insect was detected. Try another angle or capture again.")
    : getLocalizedValue(description, language, t("百科信息待补充。", "Profile pending."));
      ...
      >
        ...
        <div className="px-5 mt-2">
          <div className="rounded-2xl overflow-hidden card-shadow">
            <RemoteImage src={imageSrc} fallbackSrc={insectMantis} alt={speciesName} className="w-full h-56 object-cover" />
          </div>
        </div>
        ...
        <div className="px-5 mt-4">
          <div className="bg-card rounded-xl p-4 card-shadow micro-border">
            <h3 className="text-subtitle text-foreground mb-2">
              {isUnknown ? t("识别建议", "Recognition tips") : t("百科介绍", "Profile")}
            </h3>
